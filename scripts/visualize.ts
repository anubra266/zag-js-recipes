...
async function visualizeComponent(component: string, machineFile: string) {
    ...
    ConditionalExpression(path) {
      if (
        ...
        const alternateTarget = t.objectPattern([
          t.objectProperty(
            t.identifier("target"),
            t.stringLiteral(path.node.alternate.value)
          ),
        ]);
        const consequentTarget = t.objectPattern([
          t.objectProperty(
            t.identifier("target"),
            t.stringLiteral(path.node.consequent.value)
          ),
  ...
  traverse(ast, {
    ObjectExpression(path) {
      ...
      const isTargetWithConditionalExpression = (
        el: t.ObjectMethod | t.ObjectProperty | t.SpreadElement
      ) => targetConditions.every((cond) => cond(el as t.ObjectProperty));

      const targetWithConditionalExpression = path.node.properties.find(
        isTargetWithConditionalExpression
      );
      ...
      if (
        ...
        const testName =
          targetWithConditionalExpression.value.test.property.name;

        const guardName = `is${capitalizeFirstLetter(testName)}`;

        const otherProperties = path.node.properties.filter(
          (n) =>
            !isTargetWithConditionalExpression(n) &&
            (t.isObjectProperty(n) || t.isRestElement(n))
        ...
        const alternateTarget = t.objectPattern([
          t.objectProperty(
            t.identifier("target"),
            t.stringLiteral(
              targetWithConditionalExpression.value.alternate.value
            )
          ),
          ...otherProperties,
        ]);
        ...
        const consequentTarget = t.objectPattern([
          t.objectProperty(
            t.identifier("target"),
            t.stringLiteral(
              targetWithConditionalExpression.value.consequent.value
            )
          ),
          t.objectProperty(t.identifier("guard"), t.stringLiteral(guardName)),
          ...otherProperties,
  ...
  traverse(ast, {
    ...
    CallExpression: function (path) {
      ...
      if (
        ...
        path.node.callee.name === "createMachine"
      ) {
  ...
  ${Object.entries(delays[component]).map(([key, delay]) => `${key}: ${delay}`)}
    ...
    const guardsCode = `\n  guards: {
${guardsWithoutDuplicates
  .map((gua) => `    "${gua}": (ctx) => ctx["${gua}"],`)
  .join("    \n")}
  },`;
  ...
  ${actionsCode} ${delaysCode} ${guardsCode}
    ...
    const codeWithOptions = fillVariables(
      `${machineObjOutput.code}, \n${optionsCode}`
    );
    ...
    traverse(codeWithOptionsAst, {
      Identifier(path) {
        ...
        if (
          path.node.name === "context" &&
          t.isObjectProperty(path.parentPath.node)
        ) {
          path.parentPath.node.value = t.objectPattern(
            machineGuards.map((gua) =>
              t.objectProperty(t.stringLiteral(gua), t.booleanLiteral(false))
            )
          );
          path.stop();
        }
        ...
        if (
          ...
          t.isIdentifier(path.parentPath?.parentPath?.parentPath?.node.callee, {
            name: "createMachine",
          })
        ) {
          hasGlobalEvents = true;
          path.parentPath.node.value.properties = [
            ...path.parentPath.node.value.properties,
            updateContextNode,
          ];
        }
      ...
      traverse(codeWithOptionsAst, {
        Identifier(path) {
          ...
          if (
            ...
            const globalEventNode = t.objectProperty(
              t.identifier("on"),
              t.objectPattern([updateContextNode])
            );
...
async function visualize() {
  ...
  await Promise.all([
    modules.map((machine) => {
      const component = machine.manifest
        .name!.replace("@zag-js/", "")
        .replace("@zag-js-recipes/", "");
